...
async function checkUserStatus() {
    let userData = await getUserData(); // Fetch user data asynchronously
...
// Function to simulate fetching user data from local storage
...
// Simulate login success (call this function after successful login)
async function loginSuccess(email, gender, name) {
    const userData = {
        email: email,
        loggedIn: true,
        gender: gender,
    ...
    await saveUserData(email, userData); // Save user data asynchronously
    checkUserStatus(); // Update UI based on new login status
}

// Function to simulate saving user data to local storage
...
document.getElementById('logout').addEventListener('click', async function(event) {
    ...
    await updateUserLoginStatus(email, false); // Update the loggedIn flag
    document.getElementById('registerNow').style.display = 'block'; // Show register button
    userIcon.style.display = 'none'; // Hide user icon
...
async function updateUserLoginStatus(email, status) {
    if (localStorage.getItem(email)) { // Check if the email exists in local storage
        ...
        await saveUserData(email, userData); // Save updated user data
    }
}
...
function setupMenuHandlers() {
    ...
    document.addEventListener('click', (e) => {
        ...
        // Close mobile menu if click is outside navbar
